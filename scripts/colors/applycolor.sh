...
apply_kitty() {  
  ...
  for i in "${!colorlist[@]}"; do
    col="${colorlist[$i]//\$/\\\$}"
    sed -i "s/${col} #/${colorvalues[$i]#\#}/g" "$STATE_DIR"/user/generated/terminal/kitty-theme.conf
  done
...
apply_anyterm() {
  ...
  for i in "${!colorlist[@]}"; do
    col="${colorlist[$i]//\$/\\\$}"
    sed -i "s/${col} #/${colorvalues[$i]#\#}/g" "$STATE_DIR"/user/generated/terminal/sequences.txt
  done

...
AbstractBackgroundWidget {
    ...
    property string widgetConfigName: "visualizer"
    configEntryName: widgetConfigName
    ...
    readonly property real barsHeight: bandHeight

    implicitWidth: isRing ? ringSize : screenWidth
    implicitHeight: isRing ? ringSize : bandHeight
    width: implicitWidth
    height: implicitHeight
    x: isRing ? targetX : 0
    y: isRing ? targetY : screenHeight - height
    z: isRing ? (configEntry.z ?? 15) : 1
    draggable: isRing && placementStrategy === "free" && !Config.options.background.widgetsLocked
    hoverEnabled: isRing
    acceptedButtons: isRing ? (Qt.LeftButton | Qt.RightButton) : Qt.NoButton
    showSelectionBorder: isRing && !Config.options.background.widgetsLocked

    function restoreXYBinding() {
        root.x = Qt.binding(() => root.isRing ? root.targetX : 0);
        root.y = Qt.binding(() => root.isRing ? root.targetY : root.screenHeight - root.height);
        root.z = Qt.binding(() => root.isRing ? (root.targetZ ?? 15) : 1);
    }
    ...
    ResizeHandler {
        ...
        resizeMode: "diagonal"
        z: 2
        onResized: newValue => root.ringSizeOverride = Math.round(Math.min(Math.max(newValue, 150), 900))
        onResizeFinished: {
    ...
    component BarsVisualizer: Item {
        ...
        property real maxBarHeight: Math.max(30, root.bandHeight - 20)
        ...
        property real activityOpacity: points.some(p => p > 0) ? 1.0 : 0
            ...
            Repeater {
                ...
                Rectangle {
                    ...
                    property real pointValue: {
                        ...
                        return Math.max(bars.barWidth, (v / bars.maxVisualizerValue) * (root.configEntry.sensitivity ?? 1) * bars.maxBarHeight)
                    }
                    height: pointValue
                    ...
                    property color c1: root.visualizerColors[0]
                    property color c2: root.visualizerColors[1]
                    color: Qt.rgba(
                        c1.r * intensity + c2.r * (1 - intensity),
                        c1.g * intensity + c2.g * (1 - intensity),
                        c1.b * intensity + c2.b * (1 - intensity),
                        1
                    )
